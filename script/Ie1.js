$(document).ready(function() {
    ...
    $('#btn').click(function() {
        ...
        if ($('#btn').text() == 'Ajouter') {
            $.ajax({
                url: 'controleur/gestionIe1.php',
                data: { op: 'add', stMer: stMer.val(), stCond: stCond.val(), etVap: etVap.val(), hr: hr.val(), chef: chef.val(), operateur: operateur.val(), date: date.val() },
                type: 'POST',
                success: function(data, textStatus, jqXHR) {
                    remplir(data);
                    date.val('');
    ...
    $(document).on('click', '.modifier', function() {
        ...
        var etVap = $(this).closest('tr').find('td').eq(4).text();
        ...
        btn.click(function() {
            if ($('#btn').text() == 'Modifier') {
                $.ajax({
                    url: 'controleur/gestionIe1.php',
                    data: { op: 'update', id: $("#id").val(), stMer: $("#stMer").val(), stCond: $("#stCond").val(), etVap: $("#etVap").val(), hr: $("#hr").val(), chef: $("#chef").val(), operateur: $("#operateur").val(), date: $("#date").val() },
                    type: 'POST',
                    success: function(data, textStatus, jqXHR) {
                        remplir(data);
                        $("#date").val('');
