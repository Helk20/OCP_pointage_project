$(document).ready(function() {
    ...
    $('#btn').click(function() {
        ...
        if ($('#btn').text() == 'Ajouter') {
            $.ajax({
                url: 'controleur/gestionIp2.php',
                data: { op: 'add', nivH: nivH.val(), pFil: pFil.val(), pAspi: pAspi.val(), amp: amp.val(), hr: hr.val(), chef: chef.val(), operateur: operateur.val(), date: date.val() },
                type: 'POST',
                success: function(data, textStatus, jqXHR) {
                    remplir(data);
                    date.val('');
    ...
    $(document).on('click', '.modifier', function() {
        ...
        var pFil = $(this).closest('tr').find('td').eq(6).text();
        var nivH = $(this).closest('tr').find('td').eq(7).text();
        btn.text('Modifier');
        $("#date").val(date);
        ...
        btn.click(function() {
            if ($('#btn').text() == 'Modifier') {
                $.ajax({
                    url: 'controleur/gestionIp2.php',
                    data: { op: 'update', id: $("#id").val(), nivH: $("#nivH").val(), pFil: $("#pFil").val(), pAspi: $("#pAspi").val(), amp: $("#amp").val(), hr: $("#hr").val(), chef: $("#chef").val(), operateur: $("#operateur").val(), date: $("#date").val() },
                    type: 'POST',
                    success: function(data, textStatus, jqXHR) {
                        remplir(data);
                        $("#date").val('');
