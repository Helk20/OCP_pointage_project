$(document).ready(function() {
    ...
    $('#btn').click(function() {
        ...
        if ($('#btn').text() == 'Ajouter') {
            $.ajax({
                url: 'controleur/gestionIp6.php',
                data: { op: 'add', tSor: tSor.val(), tpali: tpali.val(), nivH: nivH.val(), pFil: pFil.val(), pEqui: pEqui.val(), pRefou: pRefou.val(), pAspi: pAspi.val(), amp: amp.val(), hr: hr.val(), chef: chef.val(), operateur: operateur.val(), date: date.val() },
                type: 'POST',
                success: function(data, textStatus, jqXHR) {
                    remplir(data);
                    date.val('');
    ...
    $(document).on('click', '.modifier', function() {
        ...
        var pEqui = $(this).closest('tr').find('td').eq(7).text();
        var pFil = $(this).closest('tr').find('td').eq(8).text();
        var nivH = $(this).closest('tr').find('td').eq(9).text();
        var tpali = $(this).closest('tr').find('td').eq(10).text();
        var tSor = $(this).closest('tr').find('td').eq(11).text();
        btn.text('Modifier');
        $("#date").val(date);
        ...
        btn.click(function() {
            if ($('#btn').text() == 'Modifier') {
                $.ajax({
                    url: 'controleur/gestionIp6.php',
                    data: { op: 'update', id: $("#id").val(), tSor: $("#tSor").val(), tpali: $("#tpali").val(), nivH: $("#nivH").val(), pFil: $("#pFil").val(), pEqui: $("#pEqui").val(), pRefou: $("#pRefou").val(), pAspi: $("#pAspi").val(), amp: $("#amp").val(), hr: $("#hr").val(), chef: $("#chef").val(), operateur: $("#operateur").val(), date: $("#date").val() },
                    type: 'POST',
                    success: function(data, textStatus, jqXHR) {
                        remplir(data);
                        $("#date").val('');
    ...
    function remplir(data) {
        ...
        for (i = 0; i < data.length; i++) {
            ...
            ligne += '<td>' + data[i].pEqui + '</td>';
            ligne += '<td>' + data[i].pFil + '</td>';
            ligne += '<td>' + data[i].nivH + '</td>';
            ligne += '<td>' + data[i].tpali + '</td>';
            ligne += '<td>' + data[i].tSor + '</td>';
